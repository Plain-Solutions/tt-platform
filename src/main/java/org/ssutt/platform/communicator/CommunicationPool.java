...
public class CommunicationPool {
    ...
    public static void setJSONHandler(JSONHandler jsh) {
        CommunicationPool.jsh = jsh;
    }

    public static CommunicationPool getInstance() {
        if (cp==null){
            cp = new CommunicationPool();
        }
        return cp;
    }

    public TTDataCommunicator getDCinstance() {
        return new TTDataCommunicator(dm, jsh);
    }
